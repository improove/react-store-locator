  ...
  useEffect(() => {
    if (containerRef.current) {
      createStoreLocatorMap({
        ...
        searchBoxOptions,
        storeListOptions
      }).then(_onMapInit).catch(err => console.error('Could not initialize store locator map.', err));
    }
  }, [loaderOptions, geoJson, formatLogoPath, infoWindowOptions, mapOptions, searchBoxOptions, storeListOptions, _onMapInit]); // Include a default minHeight to prevent confusion over why the map is not visible if no
  // styling is provided.

  const defaultHeight = className ? undefined : {
    minHeight: 400
  };
